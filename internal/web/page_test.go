...
// The body/data typeface is Sora, served from the vendored stylesheet
// (no external font CDN).
func TestPageUsesSoraAsBodyFont(t *testing.T) {
	body := get(t, seeded(t, match("wc-1")), now, "/").Body.String()

	if !strings.Contains(body, "--body-font: Sora") {
		t.Errorf("page should default the body font to Sora: %s", body)
	}
	if strings.Contains(body, "fonts.googleapis.com") {
		t.Errorf("fonts must be self-hosted, not loaded from a CDN: %s", body)
	}
}
